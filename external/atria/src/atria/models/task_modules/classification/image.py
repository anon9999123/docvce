    ...
    def visualization_step(
        ...
        **kwargs,
        ...
        if idist.get_rank() == 0:
            ...
            for label_name in self._dataset_metadata.labels:
                indices = label == self._dataset_metadata.labels.index(label_name)
                input = image[indices]
                num_samples = input.shape[0]
                if num_samples > 0:
                    self._tb_logger.writer.add_image(
                        f"visualization/{label_name}",
                        torchvision.utils.make_grid(
                            input, nrow=int(math.sqrt(num_samples))
                        ),
                        global_step,
                    )
