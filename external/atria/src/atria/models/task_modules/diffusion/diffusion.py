...
def _get_input_shape(model: nn.Module):
...
class DiffusionModule(AtriaTaskModule):
    ...
    def _build_model(
        ...
        self._input_shape = _get_input_shape(
            model.trainable_models.reverse_diffusion_model
        )
    ...
    def save_outputs(
        self, generated_samples: torch.Tensor, batch: BatchDict, model_kwargs: dict
    ) -> None:
        output_dir = (
        ...
        if self._use_cfg and self._guidance_scale > 1.0:
            output_dir = output_dir / f"cfg-{self._guidance_scale}"
        else:
            output_dir = output_dir / f"cfg-null"
        ...
        for idx, sample in enumerate(generated_samples):
            if "class_labels" in model_kwargs:
                ...
                sample_output_dir = output_dir / f"{class_label}"
            else:
                sample_output_dir = output_dir
            sample_output_dir.mkdir(exist_ok=True, parents=True)
            torchvision.utils.save_image(
                sample,
                sample_output_dir / f"{batch['__key__'][idx]}.png",
                normalize=True,
                nrow=1,
            )
    ...
    def evaluation_step(self, batch: BatchDict, **kwargs) -> ModelOutput:
        ...
        # save outputs
        if kwargs["stage"] == TrainingStage.test:
            self.save_outputs(
    ...
    def visualization_step(
        ...
        # plt.imshow(generated_samples[0].permute(1, 2, 0).cpu().detach().numpy())
        # plt.show()
        # exit()
