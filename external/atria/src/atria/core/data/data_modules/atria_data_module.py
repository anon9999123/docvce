...
class AtriaDataModule(ABC):
    def __init__(
        ...
        data_labels_override: Optional[List[str]] = None,
        combine_train_and_test_for_test: bool = False,
    ):
        ...
        self._data_labels_override = data_labels_override
        self._combine_train_and_test_for_test = combine_train_and_test_for_test
        ...
        if self._combine_train_and_test_for_test:
            self._use_train_set_for_test = False
    ...
    def _load_test_dataset(self, disable_subsampling: bool = False):
        ...
        test_dataset = self._prepare_data_transforms(
            ...
            datasets.Split.TEST,
            max_samples=(
                self._max_test_samples
                if not self._combine_train_and_test_for_test
                else None
            ),
            disable_subsampling=disable_subsampling,
        )
    ...
    def _prepare_datasets_for_stage(
        ...
        if (
            ...
            or self._combine_train_and_test_for_test
        ):
    ...
    def setup(
        ...
        self._dataset_metadata.labels = (
            self._data_labels_override or self._dataset_metadata.labels
        )
        if self._combine_train_and_test_for_test and not disable_subsampling:
            self._test_dataset = torch.utils.data.ConcatDataset(
                [self._train_dataset, self._test_dataset]
            )
            self._test_dataset = (
                _create_random_subset(self._test_dataset, self._max_test_samples)
                if self._max_test_samples is not None
