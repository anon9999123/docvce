...
class MsgpackShardListDataset(Dataset):
    ...
    def load_features(self, features_key: str) -> None:
        ...
        try:
            ...
            assert len(self._features_metadata) == len(
                self._data
            ), f"Number of features metadata rows ({len(self._features_metadata)}) does not match number of data files ({len(self._data)})"
