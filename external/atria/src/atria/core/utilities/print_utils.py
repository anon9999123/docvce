...
def _print_batch_info(batch):
    logger.info("Printing batch info:")
    logger.info("Keys in batch: {}".format(batch.keys()))
    if isinstance(batch, dict):
        for key, value in batch.items():
            msg_str = f"Batch element={key}, "
            if isinstance(value, np.ndarray):
                logger.info(msg_str + _np_array_info_str(value))
            elif isinstance(value, torch.Tensor):
                logger.info(msg_str + _tensor_info_str(value))
            elif isinstance(value, list):
                if isinstance(value[0], np.ndarray):
                    logger.info(msg_str + _np_array_info_str(value[0]))
                elif isinstance(value[0], torch.Tensor):
                    logger.info(msg_str + _tensor_info_str(value[0]))
                else:
                    logger.info(msg_str + _list_element_info_str(value))
            else:
                logger.info(msg_str + f"type={type(value)}\nExample: {value}")
...
def _print_output_info(output: ModelOutput):
    ...
    for field in dataclasses.fields(output):
        ...
        if isinstance(value, np.ndarray):
            ...
            logger.info(msg_str + _tensor_info_str(value.detach().cpu()))
        elif isinstance(value, list):
            if isinstance(value[0], np.ndarray):
                logger.info(msg_str + _np_array_info_str(value[0]))
            elif isinstance(value[0], torch.Tensor):
                logger.info(msg_str + _tensor_info_str(value[0].detach().cpu()))
            else:
                logger.info(msg_str + _list_element_info_str(value))
        else:
            logger.info(msg_str + f"type={type(value)}\nExample: {value}")
