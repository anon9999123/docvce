...
DIFFUSION_MODEL_CHECKPOINT=output/atria_trainer/Tobacco3482/classifier_guidance_klf4/checkpoints/checkpoint_43500.pt
MODEL=(
    resnet50
    convnext_base
...
CLASSIFIER_MODEL_CHECKPOINT=(
    pretrained_models/tobacco3482/resnet50.pt
    pretrained_models/tobacco3482/convnext_base.pt
    pretrained_models/tobacco3482/dit_b.pt
)
sampling_types=(
    guided_ddpm
    guided_ddim
...
for i in "${!MODEL[@]}"; do
    ...
    for sampling_type in "${sampling_types[@]}"; do
        PYTHONPATH=$PYTHONPATH TASK=document_classification DATASET=tobacco3482 python ./external/atria/src/atria/core/task_runners/atria_model_evaluator.py \
            ...
            task_module.classifier_gradient_weight=0.8 \
            task_module.distance_gradient_weight=0.2 \
            data_module.max_test_samples=1000 \
            task_module.target_query_labels_path=$BASE_PATH/notebooks/class_similarity/tobacco3482/top_5_closest_classes.csv \
            +data_module.combine_train_and_test_for_test=True \
            $@
    done
done
